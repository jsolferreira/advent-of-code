const input = '';

const binaryData = hexaToBinary(input);

console.log('Result: ', parsePacket(binaryData).decimalLiteral);

function hexaToBinary(binary) {
    const dic = {
        '0': '0000',
        '1': '0001',
        '2': '0010',
        '3': '0011',
        '4': '0100',
        '5': '0101',
        '6': '0110',
        '7': '0111',
        '8': '1000',
        '9': '1001',
        'A': '1010',
        'B': '1011',
        'C': '1100',
        'D': '1101',
        'E': '1110',
        'F': '1111'
    };

    return binary.split('').map(x => dic[x]).join('');
}

function parsePacket(data = '', previousPacketType = null) {
    const packetType = binaryToDecimal(data.slice(3, 6));

    if (packetType === 4) {
        return parseLiteralValue(data, previousPacketType);
    } else {
        const lengthType = data[6];

        if (lengthType === '0') {
            const lengthOfSubPackets = binaryToDecimal(data.slice(7, 22));

            let { decimalLiteral, nextPacket } = parsePacket(data.slice(22, 22 + lengthOfSubPackets), packetType);

            while (nextPacket !== '' && binaryToDecimal(nextPacket) !== 0) {
                const parsePacketResult = parsePacket(nextPacket, packetType);
                nextPacket = parsePacketResult.nextPacket
                decimalLiteral = applyPacketRules(packetType, decimalLiteral, parsePacketResult.decimalLiteral).decimalLiteral;
            }

            return { decimalLiteral, nextPacket: data.slice(22 + lengthOfSubPackets) };
        } else if (lengthType === '1') {

            let numberOfSubPackets = binaryToDecimal(data.slice(7, 18));
            const remaining = data.slice(18);
            let { decimalLiteral, nextPacket } = parsePacket(remaining, packetType);

            while (numberOfSubPackets-- > 1 && nextPacket !== '' && binaryToDecimal(nextPacket) !== 0) {
                const parsePacketResult = parsePacket(nextPacket, packetType);
                nextPacket = parsePacketResult.nextPacket
                decimalLiteral = applyPacketRules(packetType, decimalLiteral, parsePacketResult.decimalLiteral).decimalLiteral;
            }

            return { decimalLiteral, nextPacket };
        }
    }
}

function parseLiteralValue(value) {
    let i = 6;
    let binaryLiteral;
    let group = value.slice(i, i + 5);
    binaryLiteral = group.slice(1);

    while (group[0] !== '0') {
        i += 5;
        group = value.slice(i, i + 5);
        binaryLiteral += group.slice(1);
    }
    const decimalLiteral = binaryToDecimal(binaryLiteral);

    const nextPacket = value.slice(i + 5);

    return {
        decimalLiteral,
        nextPacket
    };
}

function applyPacketRules(packetType, op1, op2) {
    if (packetType === 0) {
        return { decimalLiteral: op1 + op2 }
    }
    if (packetType === 1) {
        return { decimalLiteral: op1 * op2 }
    }
    if (packetType === 2) {
        return { decimalLiteral: Math.min(op1, op2) }
    }
    if (packetType === 3) {
        return { decimalLiteral: Math.max(op1, op2) }
    }
    if (packetType === 5) {
        return { decimalLiteral: op1 > op2 }
    }
    if (packetType === 6) {
        return { decimalLiteral: op1 < op2 }
    }
    if (packetType === 7) {
        return { decimalLiteral: op1 == op2 }
    }
}

function binaryToDecimal(binary = '') {
    let decimal = 0;

    for (let j = binary.length - 1; j >= 0; j--) {
        if (binary[j] === '1') {
            decimal += Math.pow(2, binary.length - 1 - j)
        }
    }

    return decimal;
}
